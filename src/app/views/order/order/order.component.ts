...
export class OrderComponent implements OnInit {
    ...
    this.cartService.getCart()
      .subscribe((data: CartType | DefaultResponseType) => {
        ...
        if(!this.cart || (this.cart && this.cart.items.length === 0)) {
          this._snackBar.open('The cart is empty');
  ...
  updateDeliveryTypeValidation() {
    ...
    // Rewriting the validation in case of delivery
    ...
    // Using method updateValueAndValidity method
  ...
  createOrder() {
    if(
      this.orderForm.valid && this.orderForm.value.firstName &&
        ...
        .subscribe({
          ...
          error: (errorResponse: HttpErrorResponse) => {
            if (errorResponse.error && errorResponse.error.message) {
              ...
              this._snackBar.open('Error during the order')
            }
          }
        })
    } else {
      this.orderForm.markAllAsTouched();
      this._snackBar.open('Fill the needed fields');
    }
  }
